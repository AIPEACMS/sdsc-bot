...
- **Role grids.** Console command-only controls no longer appear in the grid.
  Global admins now see their inherited normal-admin controls in blue, their
  member controls in green, and only global-admin-specific controls in red.
...
- **Unhold works while messages are held.** `unhold` now immediately reopens
  the message gate before replying, instead of trying to send a confirmation
  that the hold gate then dropped.
...
- **Global allocation visibility.** `all-status` continues to show checker
  allocations; only `group-status` filters allocation entries to its group.
